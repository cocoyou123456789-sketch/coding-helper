  ...
  return (
    <main className="legal-shell">
      <article className="legal-paper">
        ...
        <section>
          <h2>找不到笔记或进度</h2>
          <p>学习数据只保存在当前设备，不会自动同步。删除、卸载或换机前，请在“设置 → 完整备份”导出 JSON 文件；在另一台设备上选择“从文件恢复”即可带回代码、逐行解释、复盘、课程听写、进度和偏好。恢复会完整替换本机数据，不会合并，学习提醒也会保持关闭。</p>
        </section>

        <section>
          <h2>备份文件无法恢复</h2>
          <p>请选择题解簿导出的原始 JSON 文件，不要修改扩展名或文件内容。为了避免页面卡住，当前版本只接受 24 MB 以内的备份。恢复前会显示备份时间、题目数、课程数和 XP；在你确认替换前，本机数据不会改变。</p>
        </section>
